...
is not approximately equal to the true value of 𝜇, especially for
smaller effect sizes. When we only look at the cases that achieved
statistical significance, we’re essentially selecting the samples where
...
city <- homicides %>%
  ...
  select(city_state, total_homicides, unsolved_homicides, tidy) %>%
  unnest(tidy) %>%
...
```
    ...
    ## Rows: 51
    ## Columns: 6
    ## $ city_state         <chr> "Albuquerque, NM", "Atlanta, GA", "Baltimore, MD", …
    ## $ total_homicides    <int> 378, 973, 2827, 424, 800, 614, 521, 687, 5535, 694,…
    ## $ unsolved_homicides <int> 146, 373, 1825, 196, 347, 310, 319, 206, 4073, 309,…
    ## $ estimate           <dbl> 0.3862434, 0.3833505, 0.6455607, 0.4622642, 0.43375…
    ## $ conf.low           <dbl> 0.3372604, 0.3528119, 0.6275625, 0.4141987, 0.39918…
    ## $ conf.high          <dbl> 0.4375766, 0.4148219, 0.6631599, 0.5110240, 0.46895…
...
ggplot(city, aes(x = reorder(city_state, estimate), y = estimate)) +
  ...
  coord_flip() +
  labs(
    title = "Proportion of Unsolved Homicides by City (2007–2017)",
    x = NULL,
...
![](p8105_hw5_xc2784_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
The estimated proportion of unsolved homicides varies widely across
cities. Chicago and Baltimore have the highest proportions, while
Richmond and Tulsa have the lowest.
